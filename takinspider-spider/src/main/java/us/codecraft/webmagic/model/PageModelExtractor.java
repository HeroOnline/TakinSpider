...
class PageModelExtractor {
    ...
    //提取TARGET的正则表达式集合
    private List<Pattern> targetUrlPatterns = Lists.newArrayList();

    private Selector targetUrlRegionSelector;

    private List<Pattern> helpUrlPatterns = Lists.newArrayList();
    ...
    private void initClassExtractors() {
        ...
        if (annotation == null) {
            ...
            for (String s : value) {
                String sr = s;// s.replace(".", "\\.");//.replace("*", "[^\"'#]*");
                targetUrlPatterns.add(Pattern.compile("(" + sr + ")"));
            }
    ...
    public Object process(Page page) {
        ...
        for (Pattern targetPattern : targetUrlPatterns) {
            logger.info("parrent " + targetPattern.toString());
